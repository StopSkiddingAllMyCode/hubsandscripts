...
runcode(function()
	...
	HackerDetector = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		Name = "HackerDetector",
		HoverText = "Detects blatant hackers",
		Function = function(callback)
			if callback then
                enable2 = true
                if enable2 == true then
                    local function checkCharacter(character)
                        ...
                        if humanoid and rootPart then
                            ...
                            if speed > 23 then
                                createwarning("HackerDetector", "Hacker detected: "..character.Name.." has a speed over 23 CFrame", 5)
                            end
                        end
                        if humanoid and humanoid:GetState() == Enum.HumanoidStateType.Physics and humanoid.FloorMaterial == Enum.Material.Air then
                            createwarning("HackerDetector", "Hacker detected: "..character.Name.." is flying", 5)
                        end
                        if rootPart and rootPart.Position.Y > 100 then
                            createwarning("HackerDetector", "Hacker detected: "..character.Name.." is a lot of studs in the air", 5)
                        end
                    end
